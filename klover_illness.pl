...
definition(foregn_interest,'Foreign interests comment').
...
rule_info(investment_bonds,[200,201,202,203]).
rule_info(investment_metals_gemstones,[210,211,212]).
rule_info(investment_other,[220,221]).
...
rule(42,investment_objects, tax_deductible_saving_account,500):-
	check(riskwillingness,=,reasonable),
	check(accessibility,=,yes),
	check(prognosis,=,recession).	
% rules about art
...
rule(80,investment_objects,antiques,600):-
	check(interest,=,antiques),
        check(experience,=,antiques),
	check(contacts,=,antiques).

rule(90,foregn_fund,600):-
	check(foregn_interest,=,yes),
	check(accessibility,=,no).
rule(100,pension,400):-
	check(accessibility,=,no),
	check(government,\=,'left').
rule(101,pension,700):-
	check(age,>=,60),
	check(government,\=,'left'),
	check(accessibility,=,yes).
rule(102,pension,300):-
	check(accessibility,=,no),
	check(government,=,'left').
rule(103,pension,600):-
	check(age,>=,60),
	check(government,=,'left'),
	check(accessibility,=,yes).
% rules about what to invest in in regarding stock, bonds and index_options

rule(200,investment_bonds,'combine stock and bonds',900):-
        check(investment_objects,stock,'cf>=',700),
        check(investment_objects,bonds,'cf>=',700).

rule(201,investment_bonds,stock,500):-
        check(investment_objects,stock,'cf>=',600),
        check(investment_objects,bonds,'cf<',700).

rule(202,investment_bonds,stock,300):-
        check(investment_objects,stock,'cf>=',500),
        check(investment_objects,index_options,'cf<',400).

rule(203,investment_bonds,index_options,500):-
        (check(investment_objects,stock,'cf<',700);
        check(investment_objects,bonds,'cf<',700)),
        check(investment_objects,index_options,'cf>=',400).
rule(204,investment_bonds,foregn_fund,600):-
	check(investment_objects,foregn_fund,'cf>',600),
	check(investment_objects,bonds,'cf<',700).


% rules about what to invest regarding gold and diamands        
rule(210,investment_metals_gemstones,'combine diamonds and gold',1000):-
        check(investment_objects,diamonds,'cf>=',600),
        check(investment_objects,gold,'cf>=',600).

rule(211,investment_metals_gemstones,diamonds,700):-
        check(investment_objects,diamonds,'cf>=',500),
        check(investment_objects,gold,'cf<',500).
rule(212,investment_metals_gemstones,gold,700):-
        check(investment_objects,gold,'cf>=',500),
...
% rules about what to invest regarding other investments, such as art and tax_deductible_saving_account
rule(220,investment_other,art,600):-
        check(investment_objects,art,'cf>=',600).
rule(221,investment_other,tax_deductible_savings_account,600):-
        check(investment_objects,tax_deductible_savings_account,'cf>=',500).
rule(222,investment_other,(antiques,other),800):-
	check(investment_objects,art,'cf>=',600).
rule(223,investment_other,antiques,600):-
	check(investment_objects,'cf>=',600),
	check(art,'cf<',600).
rule(224,investment_other,pension,'cf>=',700):-
	check(pension,'cf>',400).
...
present_setup(Presentationsparametrar):-		
        ...
        presentation(Presentationsparametrar),
		Source = 'source C:/Users/boron/Desktop/klover/conclusion.tcl',
		run_tcl(Source,Event),
		retract_and_destroy(Event).
present_setup(Presentationsparametrar):-
		tcl_GUI(on),
...
write_no_result:-
		tcl_GUI(on),
		Source = 'source C:/Users/boron/Desktop/klover/no_conclusion.tcl',
		run_tcl(Source, Event),
		retract_and_destroy(Event).
...
arrange_how_list(List_of_answers, How_object):-
		...
		assert_each_option(List_of_answers),
		assert_num_options(List_of_answers),
		Source = 'source C:/Users/boron/Desktop/klover/pick_how.tcl',
		run_tcl(Source, Event),
		retract_and_destroy(Event),
		(
...
%if Body contains OR (;), check if P1 elements are members of either Bodyös left side of OR %(;),
...
go_verify('Help'):-
	tcl_GUI(on),
	Source = 'source C:/Users/boron/Desktop/klover/v_help.tcl',
	run_tcl(Source, Event),
	retract_and_destroy(Event),
	go_help(Event).
...
go_help('Redundancy'):-
	tcl_GUI(on),
	Source = 'source C:/Users/boron/Desktop/klover/r_help.tcl',
	run_tcl(Source, Event),
	retract_and_destroy(Event),
	go_help(Event).
...
go_help('Subsumed'):-
	tcl_GUI(on),
	Source = 'source C:/Users/boron/Desktop/klover/s_help.tcl',
	run_tcl(Source, Event),
	retract_and_destroy(Event),
	go_help(Event).	
...
go_help('Completeness'):-
	tcl_GUI(on),
	Source = 'source C:/Users/boron/Desktop/klover/c_help.tcl',
	run_tcl(Source, Event),
	retract_and_destroy(Event),
	go_help(Event).	
